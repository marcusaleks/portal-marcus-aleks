       ...
       <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center border-b border-red-900/30 pb-6 mb-12">
             <div className="flex items-center gap-4">
                ...
                <div><h1 className="text-white font-black text-2xl uppercase tracking-tighter leading-none font-bold">Intelligence Ops</h1><p className="text-[10px] text-red-600 font-mono uppercase tracking-[0.4em] mt-1 font-bold">Restricted Access Level 5</p></div>
             </div>
             <div className="text-right hidden md:block font-bold"><p className="text-white text-sm">{new Date().toLocaleTimeString()}</p></div>
          </div>
          <div className="grid md:grid-cols-2 gap-12 text-xs uppercase font-bold text-slate-600 tracking-widest font-bold">
             <section className="space-y-6">
                ...
                {intelStream.finance.length > 0 ? intelStream.finance.map((item: any, i: number) => (
                   <div key={i} className="p-5 border border-slate-800 bg-slate-900/20 rounded-lg hover:border-blue-500/40 transition-all font-bold"><h3 className="text-white mb-2">{item.title}</h3><a href={item.link} target="_blank" className="text-blue-500 text-[9px] hover:underline font-bold">Open Source Analysis</a></div>
