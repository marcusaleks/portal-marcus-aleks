       ...
       <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-12 text-xs uppercase font-bold text-slate-600 tracking-widest">
             <section className="space-y-6">
                ...
                {news.finance.length > 0 ? news.finance.map((item: any, i: number) => (
                   <div key={i} className="p-5 border border-slate-800 bg-slate-900/20 rounded-lg hover:border-blue-500/40 transition-all">
                      ...
                      <a href={item.link} target="_blank" className="text-blue-500 text-[9px] hover:underline">Open Source</a>
                   </div>
             ...
             <section className="space-y-6">
                ...
                {news.intel.length > 0 ? news.intel.map((item: any, i: number) => (
                   <div key={i} className="p-5 border border-slate-800 bg-red-900/5 rounded-lg hover:border-red-600/40 transition-all">
                      <h3 className="text-white mb-2">{item.title}</h3>
                      <a href={item.link} target="_blank" className="text-red-600 text-[9px] hover:underline">Analyze Threat</a>
                   </div>
