...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Chama a nova API Route para validação segura no servidor
  ...
  return (
    <div className="min-h-screen bg-[#020408] flex flex-col items-center justify-center p-6 font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-md space-y-10 z-10 animate-in fade-in zoom-in duration-1000">
        <div className="text-center space-y-6">
          ...
          <div className="space-y-2">
            <h1 className="text-white text-5xl font-black uppercase tracking-tighter italic">SIFAZ <span className="text-blue-500 text-sm not-italic font-black ml-1">v.0.0.1</span></h1>
        ...
        <div className="bg-slate-950/60 border border-slate-800 p-12 rounded-[3rem] backdrop-blur-2xl shadow-2xl space-y-10 relative overflow-hidden">
          ...
          <form onSubmit={handleLogin} className="space-y-8">
            <div className="space-y-4">
              ...
              <div className="relative group">
                ...
                <input 
                  ...
                  required
                  autoFocus
                  className={`w-full bg-black/60 border ${error ? 'border-red-600 shadow-[0_0_30px_rgba(220,38,38,0.15)]' : 'border-slate-800 focus:border-blue-600'} rounded-2xl pl-14 pr-14 py-6 text-white font-mono text-base outline-none transition-all placeholder:text-slate-900`}
            ...
            <button type="submit" disabled={loading} className={`w-full h-20 ${loading ? 'bg-slate-800' : 'bg-blue-600 hover:bg-blue-700'} text-white font-black rounded-2xl flex items-center justify-center gap-4 transition-all uppercase text-sm tracking-[0.3em] shadow-2xl`}>
      ...
      <footer className="fixed bottom-12 flex items-center gap-4 text-[11px] text-emerald-500 font-mono bg-emerald-500/5 px-8 py-4 rounded-full border border-emerald-500/10 backdrop-blur-xl">
