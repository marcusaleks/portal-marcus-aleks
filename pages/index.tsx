...
import { 
  ...
  Download, ExternalLink, BarChart3, PieChart, LineChart 
} from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchMarketData = async () => {
      try {
        // 1. Câmbio (AwesomeAPI) - Precisão de 4 casas
      ...
      } catch (err) { console.error("Falha no uplink de mercado."); }
    };
    fetchMarketData();
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-slate-300 font-sans selection:bg-blue-500/30">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .ticker-wrap { display: flex; animation: marquee 45s linear infinite; }
        .ticker-wrap:hover { animation-play-state: paused; }
      `}} />

      {/* BANNER ANIMADO: 10 PRINCIPAIS AÇÕES */}
         ...
         <div className="ticker-wrap gap-12 items-center flex whitespace-nowrap">
            {market.stocks.length > 0 ? [...market.stocks, ...market.stocks].map((stock: any, i) => (
              <div key={i} className="flex items-center gap-3 px-4 border-r border-slate-800/50">
                <span className="text-[10px] font-black text-white uppercase">{stock.symbol}</span>
            ...
            )) : <div className="text-[9px] font-mono uppercase animate-pulse px-6 text-slate-600">Sincronizando pregão B3 (Variação Intraday)...</div>}
         </div>
      </div>
      ...
      <header className="max-w-7xl mx-auto px-6 py-20 border-b border-slate-900/30">
        <div className="grid lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7 space-y-8">
            ...
            <p className="text-slate-500 text-xl max-w-lg leading-relaxed font-bold">Arquitetura quantitativa para monitoramento de ativos e ferramentas de cálculo financeiro aplicadas a portfólios institucionais.</p>
          </div>
          ...
          <div className="lg:col-span-5 bg-slate-950/40 border border-slate-800 p-8 rounded-2xl backdrop-blur-sm shadow-2xl space-y-8">
            ...
            <div className="flex justify-between items-center">
              <div>
                 <span className="text-[9px] font-mono text-blue-500 uppercase block mb-1 font-bold tracking-[0.2em]">SELIC EFETIVA</span>
              ...
              <div className="text-right">
                 ...
                 <span className="text-[10px] font-bold text-slate-400 tracking-tight">17/03/2026</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20 grid md:grid-cols-3 gap-8">
        <a href="https://www.tesourodireto.com.br/titulos/precos-e-taxas.htm" target="_blank" className="p-8 border border-slate-800 bg-slate-900/10 rounded-3xl group hover:border-emerald-500/40 transition-all shadow-xl">
          ...
          <p className="text-sm text-slate-500 mb-8 leading-relaxed">Consulta em tempo real de preços e taxas de títulos públicos federais (Selic, IPCA+ e Prefixados).</p>
          <div className="flex items-center gap-2 text-[10px] font-black text-emerald-500 uppercase tracking-widest group-hover:gap-4 transition-all">Ver Taxas Atuais <ArrowUpRight size={14} /></div>
        </a>

        <a href="https://www3.bcb.gov.br/CALCIDADAO/publico/exibirFormCorrecaoValores.do?method=exibirFormCorrecaoValores&aba=4" target="_blank" className="p-8 border border-slate-800 bg-slate-900/10 rounded-3xl group hover:border-blue-500/40 transition-all shadow-xl">
          ...
          <p className="text-sm text-slate-500 mb-8 leading-relaxed">Simulador oficial para correção de valores por índices de preços e atualização pela taxa Selic.</p>
          <div className="flex items-center gap-2 text-[10px] font-black text-blue-500 uppercase tracking-widest group-hover:gap-4 transition-all">Acessar Simulador <ArrowUpRight size={14} /></div>
        </a>
        ...
        <div className="p-8 border border-slate-800 bg-blue-600/5 border-blue-500/20 rounded-3xl shadow-2xl shadow-blue-900/10">
          ...
          <p className="text-sm text-slate-400 mb-8 leading-relaxed">Engine de gestão de ativos e análise quantitativa em Python para terminais financeiros locais.</p>
