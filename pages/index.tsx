...
export default function Home() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        // 3. Top 10 B3 (Brapi - Usando fallback se necessário)
        ...
        // Organizando dados para o Ticker
        const coinAssets = [
        ...
        const stockAssets = dB3.results.map(stock => ({
          t: stock.symbol,
          v: stock.regularMarketPrice.toFixed(2).replace('.', ','),
          c: (stock.regularMarketChangePercent > 0 ? '+' : '') + stock.regularMarketChangePercent.toFixed(2) + '%'
        }));
      ...
      } catch (e) { console.error("Erro na integração de dados."); }
    };
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-slate-300 font-sans selection:bg-blue-500/30 relative overflow-hidden">
      ...
      {/* BANNER DINÂMICO LARGO (py-4) COM PAUSE NO HOVER E CURSOR PADRÃO */}
      <div className="w-full bg-slate-950 border-b border-slate-800 py-4 overflow-hidden whitespace-nowrap z-[60] relative group">
        <div className="flex animate-marquee hover:[animation-play-state:paused] gap-16 items-center cursor-default">
          ...
          )) : <span className="text-[10px] font-mono text-slate-700 animate-pulse uppercase tracking-widest">Iniciando uplink de dados de mercado...</span>}
        </div>
      </div>
      ...
      <nav className="border-b border-slate-800/50 bg-[#05070a]/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <Link href="/login" className="bg-slate-900 border border-slate-700 px-4 py-2 rounded text-[10px] font-bold text-slate-400 uppercase tracking-widest hover:text-white transition-all">
          ...
          <div className="lg:col-span-5 border border-slate-800 bg-slate-950/40 p-8 rounded-2xl backdrop-blur-sm shadow-2xl relative">
            <div className="space-y-6 text-bold">
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="text-right"><span className="text-[9px] font-mono text-slate-500 uppercase block mb-1 font-bold italic">Próxima Reunião COPOM</span><span className="text-xs font-black text-slate-400 font-mono tracking-tighter">17/03/2026</span></div>
              </div>
            </div>
