...
export default function Home() {
    ...
    const runUplink = async () => {
      try {
        ...
        setTickerData([...b3, 
          ...
          { t: 'EUR/BRL', v: d1.EURBRL.bid.replace('.', ','), c: (parseFloat(d1.EURBRL.pctChange) > 0 ? '+' : '') + d1.EURBRL.pctChange + '%' },
          { t: 'BTC/BRL', v: parseFloat(d1.BTCBRL.bid).toLocaleString('pt-BR'), c: (parseFloat(d1.BTCBRL.pctChange) > 0 ? '+' : '') + d1.BTCBRL.pctChange + '%' },
          { t: 'ETH/BRL', v: parseFloat(d1.ETHBRL.bid).toLocaleString('pt-BR'), c: (parseFloat(d1.ETHBRL.pctChange) > 0 ? '+' : '') + d1.ETHBRL.pctChange + '%' }
        ]);
      ...
      } catch (e) { console.log("Erro no uplink."); }
    };
    runUplink();
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-slate-300 font-sans selection:bg-blue-500/30 relative overflow-hidden">
      ...
      {/* 1. Banner Ultra-Largo (py-8) e Dinâmico */}
      ...
      <nav className="border-b border-slate-800/50 bg-[#05070a]/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center font-bold">
          <div className="flex items-center gap-3">
            <div className="bg-blue-600 px-2 py-0.5 rounded font-black text-white text-[11px] tracking-tighter shadow-lg">MAD</div>
      ...
      <header className="max-w-7xl mx-auto px-6 py-20 border-b border-slate-900/50">
        <div className="grid lg:grid-cols-12 gap-12 items-start font-bold">
          <div className="lg:col-span-7 space-y-8">
            <h1 className="text-6xl md:text-8xl font-black leading-none text-white tracking-tighter uppercase italic tracking-tighter">Mercado <br/><span className="text-blue-500 not-italic uppercase">Capitais</span></h1>
          ...
          <div className="lg:col-span-5 border border-slate-800 bg-slate-950/40 p-8 rounded-2xl backdrop-blur-sm shadow-2xl relative">
            ...
            <div className="mt-8 pt-4 border-t border-slate-900 text-[9px] font-mono flex justify-between uppercase font-black text-slate-600 tracking-widest font-bold">
      ...
      {/* RESTAURADO: MARKET INTELLIGENCE (Notícias) */}
      ...
      <section className="max-w-7xl mx-auto px-6 py-16 grid md:grid-cols-3 gap-8">
        <div className="p-8 border border-slate-800 bg-slate-900/10 rounded-2xl group hover:border-blue-500/40 transition-all">
          ...
          <a href="https://www.tesourodireto.com.br/titulos/precos-e-taxas.htm" target="_blank" className="text-[10px] font-bold text-blue-500 uppercase hover:underline flex items-center gap-2 tracking-widest font-bold">Consultar Taxas <ArrowUpRight size={14} /></a>
        </div>
        <div className="p-8 border border-slate-800 bg-slate-900/10 rounded-2xl group hover:border-blue-500/40 transition-all">
          ...
          <a href="https://www3.bcb.gov.br/CALCIDADAO/publico/exibirFormCorrecaoValores.do?method=exibirFormCorrecaoValores&aba=4" target="_blank" className="text-[10px] font-bold text-blue-500 uppercase hover:underline flex items-center gap-2 tracking-widest font-bold">Correção Monetária <ExternalLink size={14} /></a>
        </div>
        <div className="p-8 border border-slate-800 bg-blue-500/5 rounded-2xl border-blue-500/20 shadow-lg">
          ...
          <a href="https://github.com/marcusaleks/Portfolio_Manager/releases/download/v0.0.1/PortfolioManager_v0.0.1.zip" className="w-full bg-blue-600 text-white px-4 py-3 rounded text-[10px] font-black flex items-center justify-center gap-2 hover:bg-blue-700 transition-all uppercase shadow-md font-bold tracking-widest"><Download size={14} /> Download Engine</a>
        </div>
      </section>

      <footer className="max-w-7xl mx-auto px-6 py-12 border-t border-slate-900 flex flex-col md:flex-row justify-between items-center gap-8 font-bold">
