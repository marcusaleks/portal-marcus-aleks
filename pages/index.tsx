...
export default function Home() {
  const [marketData, setMarketData] = useState({ 
    usd: '5,23', eur: '6,02', selic: '15,00', ibov: '179.365', ibovChange: '-0,61%' 
  });
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        // 3. Top 10 B3 (Brapi - Nota: Use seu token se as ações sumirem novamente)
        const stocks = 'PETR4,VALE3,ITUB4,BBDC4,ABEV3,BBAS3,ITSA4,SANB11,RENT3,LREN3';
        const resB3 = await fetch(`https://brapi.dev/api/quote/${stocks}?token=`); // Insira token se disponível
        ...
        setTicker([...stockAssets, ...coinAssets]);
        setMarketData(prev => ({
          ...prev,
          usd: d.USDBRL.bid.replace('.', ','),
          eur: d.EURBRL.bid.replace('.', ','),
          selic: s[0].valor.replace('.', ',')
        }));
        setLastSync(new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }));
      } catch (e) { console.error("Falha no uplink de dados."); }
    };
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-slate-300 font-sans selection:bg-blue-500/30 relative overflow-hidden">
      ...
      {/* BANNER DINÂMICO AMPLIADO (py-6) COM PAUSE NO HOVER E CURSOR PADRÃO */}
      <div className="w-full bg-slate-950 border-b border-slate-800 py-6 overflow-hidden whitespace-nowrap z-[60] relative group shadow-2xl">
        <div className="flex animate-marquee hover:[animation-play-state:paused] gap-20 items-center cursor-default">
          {(ticker.length > 0 ? [...ticker, ...ticker] : []).map((asset, i) => (
            <div key={i} className="flex items-center gap-4 font-mono text-xs font-black tracking-tight">
      ...
      <nav className="border-b border-slate-800/50 bg-[#05070a]/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <Link href="/login" className="bg-slate-900 border border-slate-700 px-4 py-2 rounded text-[10px] font-bold text-slate-400 uppercase tracking-widest hover:text-white transition-all shadow-md hover:shadow-blue-900/20">
      ...
      <header className="max-w-7xl mx-auto px-6 py-20 border-b border-slate-900/50">
        <div className="grid lg:grid-cols-12 gap-12 items-start">
          <div className="lg:col-span-7 space-y-8">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/5 border border-blue-500/20 text-blue-500 text-[9px] font-bold uppercase tracking-[0.3em]">System Architect & Quant Analysis</div>
            ...
            <p className="text-slate-400 text-xl max-w-2xl leading-relaxed font-light">Engenharia quantitativa aplicada ao monitoramento de fluxos e arquitetura de ativos. Soluções de alta performance para o mercado financeiro brasileiro.</p>
          </div>
          ...
          <div className="lg:col-span-5 border border-slate-800 bg-slate-950/40 p-8 rounded-2xl backdrop-blur-sm shadow-2xl relative">
            <div className="space-y-6">
              <div className="flex justify-between items-center border-b border-slate-800/50 pb-5">
                <div>
                  <span className="text-[10px] font-mono text-slate-500 uppercase block mb-1 font-bold">IBOVESPA (LIVE)</span>
                  <span className="text-3xl font-bold text-white tracking-tighter">{marketData.ibov}</span>
                  <span className="text-lg text-red-500 ml-4 font-mono font-black">{marketData.ibovChange}</span>
                </div>
                <Sparkline trend="down" />
              </div>
              <div className="flex justify-between items-center border-b border-slate-800/50 pb-5">
                <div>
                  <span className="text-[10px] font-mono text-slate-500 uppercase block mb-1 font-bold tracking-widest">USD / BRL</span>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="text-right">
                   <span className="text-[9px] font-mono text-slate-500 uppercase block mb-1 font-bold">Reunião COPOM</span>
