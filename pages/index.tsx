...
export default function Home() {
  ...
  useEffect(() => {
    const fetchMarket = async () => {
      ...
      } catch (e) { console.error("Erro no uplink financeiro."); }
    };
    fetchMarket();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-slate-300 font-sans selection:bg-blue-500/30 font-bold">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .ticker-wrap { display: flex; animation: marquee 35s linear infinite; width: max-content; }
      `}} />
      ...
      <div className="w-full bg-slate-950 border-b border-slate-800 py-3 overflow-hidden z-[60] relative">
         <div className="ticker-wrap gap-12 items-center flex whitespace-nowrap">
            {[...market.stocks, ...market.stocks, ...market.stocks].map((stock: any, i) => (
      ...
      <nav className="max-w-7xl mx-auto px-6 py-8 flex justify-between items-center">
        ...
        <Link href="/login" className="bg-slate-900 border border-slate-800 px-5 py-2.5 rounded-xl text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-white hover:border-blue-600 transition-all shadow-xl">
      ...
      <header className="max-w-7xl mx-auto px-6 py-20 border-b border-slate-900/50">
        <div className="grid lg:grid-cols-12 gap-16 items-center">
          <div className="lg:col-span-7 space-y-8">
            ...
            <p className="text-slate-500 text-xl max-w-lg leading-relaxed font-bold">Arquitetura quantitativa para monitoramento de ativos e ferramentas de cálculo financeiro.</p>
          </div>

          <div className="lg:col-span-5 bg-slate-950/40 border border-slate-800 p-10 rounded-[2.5rem] backdrop-blur-sm shadow-2xl space-y-10">
            ...
            <div className="flex justify-between items-center border-b border-slate-900 pb-8">
              <div>
                 <span className="text-[10px] font-mono text-slate-500 uppercase block mb-1 font-bold">USD / BRL</span>
            ...
            <div className="flex justify-between items-center">
              <div>
                 <span className="text-[10px] font-mono text-blue-500 uppercase block mb-1 font-bold tracking-widest">SELIC EFETIVA</span>
                 <span className="text-4xl font-black text-blue-500 tracking-tighter">{market.selic}%</span>
              </div>
              <div className="text-right">
                 <span className="text-[9px] font-mono text-slate-600 uppercase block font-bold mb-1">REUNIÃO COPOM</span>
                 <span className="text-xs font-bold text-slate-400 italic">17/03/2026</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24 grid md:grid-cols-3 gap-10">
        <a href="https://www.tesourodireto.com.br/titulos/precos-e-taxas.htm" target="_blank" className="p-10 border border-slate-800 bg-slate-900/10 rounded-[2rem] group hover:border-emerald-500/40 transition-all shadow-xl">
          ...
          <div className="text-[10px] font-black text-emerald-500 uppercase tracking-widest flex items-center gap-2">Ver Taxas <ArrowUpRight size={14} /></div>
        </a>

        <a href="https://www3.bcb.gov.br/CALCIDADAO/publico/exibirFormCorrecaoValores.do?method=exibirFormCorrecaoValores&aba=4" target="_blank" className="p-10 border border-slate-800 bg-slate-900/10 rounded-[2rem] group hover:border-blue-500/40 transition-all shadow-xl">
          ...
          <div className="text-[10px] font-black text-blue-500 uppercase tracking-widest flex items-center gap-2">Simular Agora <ArrowUpRight size={14} /></div>
        </a>

        <div className="p-10 border border-slate-800 bg-blue-600/5 border-blue-500/20 rounded-[2rem] shadow-2xl">
          ...
          <a href="https://github.com/marcusaleks/Portfolio_Manager/releases/download/v0.0.1/PortfolioManager_v0.0.1.zip" 
             className="w-full bg-blue-600 text-white py-4 rounded-xl text-[11px] font-black flex items-center justify-center gap-3 hover:bg-blue-700 transition-all uppercase tracking-widest shadow-lg shadow-blue-900/20">
