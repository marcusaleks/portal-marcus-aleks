...
export default function Home() {
  const [market, setMarket] = useState({ 
    usd: '...', selic: '...', ibov: '...', ibovChange: '...', stocks: [], error: null 
  });

  useEffect(() => {
    const fetchMarketData = async () => {
      console.log("Iniciando uplink com provedores financeiros...");
      try {
        // 1. Dólar (AwesomeAPI)
        ...
        // 3. IBOVESPA e Ativos (Brapi)
        const token = process.env.NEXT_PUBLIC_BRAPI_TOKEN;
        if (!token) {
          console.warn("ALERTA: NEXT_PUBLIC_BRAPI_TOKEN não detectado no ambiente.");
        }
        ...
        if (dataB3.error || !dataB3.results) {
          throw new Error(dataB3.message || "Falha na API Brapi");
        }
        ...
        console.log("Uplink concluído com sucesso.");
      } catch (err: any) {
        console.error("ERRO CRÍTICO NO UPLINK:", err.message);
        setMarket(prev => ({ ...prev, error: err.message }));
      }
    };
    ...
    <div className="min-h-screen bg-[#05070a] text-slate-300 font-sans selection:bg-blue-500/30 font-bold">
      ...
      <div className="w-full bg-slate-950 border-b border-slate-800 py-3 overflow-hidden z-[60] relative">
         <div className="ticker-wrap gap-12 items-center flex whitespace-nowrap">
            ...
            )) : <div className="text-[9px] font-mono uppercase animate-pulse px-6 text-slate-600 italic">
                  {market.error ? `ERRO: ${market.error}` : "Sincronizando pregão B3..."}
                </div>}
         </div>
      </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-16 grid md:grid-cols-3 gap-8">
        <a href="https://www.tesourodireto.com.br/titulos/precos-e-taxas.htm" target="_blank" className="p-8 border border-slate-800 bg-slate-900/10 rounded-2xl group hover:border-emerald-500/40 transition-all">
          ...
          <p className="text-[10px] font-bold text-emerald-500 uppercase flex items-center gap-2">Preços e Taxas Atuais <ArrowUpRight size={14} /></p>
        </a>
        ...
        <a href="https://www3.bcb.gov.br/CALCIDADAO/publico/exibirFormCorrecaoValores.do?method=exibirFormCorrecaoValores&aba=4" target="_blank" className="p-8 border border-slate-800 bg-slate-900/10 rounded-2xl group hover:border-blue-500/40 transition-all">
          ...
          <p className="text-[10px] font-bold text-blue-500 uppercase flex items-center gap-2">Correção de Valores <ArrowUpRight size={14} /></p>
        </a>
